    ...
    ILogger<VmsDbContextSeeder> logger)
{
    ...
    public async Task SeedAsync(IWebHostEnvironment env, IOptions<AppSettings> settings)
    {
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            try
            {
                // ensure this user (migrator) exists
        ...
        async Task SeedData()
        {
            ...
            foreach (var ci in Enumerable.Range(1, 5))
            {
                ...
                foreach (var fi in Enumerable.Range(1, 50))
                {
                    ...
                    var fleet1 = await New<CreateFleet>()
                ...
                foreach (var ni in Enumerable.Range(1, 20))
                {
                    ...
                    var fleet = await New<CreateNetwork>()
                ...
                foreach (var csi in Enumerable.Range(1, 100))
                {
                    ...
                    var customer = await New<CreateCustomer>()
            ...
            foreach (var driverInfo in GetDrivers())
            {
                ...
                var vehicle = await New<CreateVehicle>()
                ...
                var driver = await New<CreateDriver>()
        ...
        void SeedLists()
        {
            foreach (var company in context.ChangeTracker.Entries<Company>().Select(x => x.Entity).ToList())
            {
                for (int i = 0; i < 5; i++)
                {
                    context.NotCompleteReasons.Add(
                        new NotCompleteReason(company.Code, $"CODE{i:D2}", $"Not Complete Reason #{i}"));

                    context.ConfirmBookedRefusalReasons.Add(
                        new ConfirmBookedRefusalReason(company.Code, $"CODE{i:D2}", $"ConfirmBooked Refusal reason #{i}"));

                    context.NonArrivalReasons.Add(
                        new NonArrivalReason(company.Code, $"CODE{i:D2}", $"NonArrival Reason reason #{i}"));

                    context.RefusalReasons.Add(
                        new RefusalReason(company.Code, $"CODE{i:D2}", $"Refusal reason #{i}"));

                    context.RescheduleReasons.Add(
