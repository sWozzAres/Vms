...
public class TestDatabaseFixture
{
    private const string ConnectionString = 
        "Server=SKYLAKE\\SQL2016;Database=VehicleManagementSystem;Trusted_Connection=True;MultipleActiveResultSets=true;Encrypt=False";
        ...
        lock (_lock)
        {
            if (!_databaseInitialized)
            {
                using (var context = CreateContext())
                {
                    ...
                    // seed
                    var vehicle1 = new Vehicle("S684FDU", make1.Make, model1_1.Model, new DateOnly(2000, 1, 14), new DateOnly(2000, 1, 14), new List<Driver>()
                    {
